...
function BeerApp() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-300">
      <div className="container mx-auto p-6 max-w-4xl">
        ...
        {/* Stats Card */}
        <Card className="mb-8 border-amber-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl text-amber-900 dark:text-amber-100">
              Total Beers Tracked
            </CardTitle>
            <div className="text-5xl font-bold text-amber-600 dark:text-amber-400 mt-2">
              {beerCount.count}
            </div>
            {isBackendDown && (
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="flex gap-4">
              <Input
                ...
                className="flex-1 border-amber-200 dark:border-slate-600 focus:border-amber-400 dark:focus:border-amber-500 bg-white dark:bg-slate-700 text-amber-900 dark:text-amber-100 placeholder:text-amber-500 dark:placeholder:text-slate-400"
                required
              />
              <Button 
                type="submit" 
        ...
        {/* Beer List */}
        <Card className="border-amber-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-amber-900 dark:text-amber-100 flex items-center gap-2">
              <span>📋</span>
          ...
          <CardContent>
            {isLoading ? (
              <div className="text-center py-8">
                <div className="text-amber-600 dark:text-amber-400">Loading your beers...</div>
              </div>
            ) : beers.length === 0 ? (
              <div className="text-center py-8 text-amber-600 dark:text-amber-400">
                <div className="text-6xl mb-4">🍺</div>
                <p className="text-lg mb-2">No beers tracked yet!</p>
                <p className="text-sm text-amber-500 dark:text-slate-500">
                  Add your first beer using the form above
                </p>
                {isBackendDown && (
                  <p className="text-xs text-amber-400 dark:text-slate-400 mt-3">
              ...
              <div className="space-y-3">
                {beers.map((beer: Beer, index: number) => (
                  <div key={beer.id}>
                    <div className="flex items-center justify-between p-4 rounded-lg bg-amber-50 dark:bg-slate-700 border border-amber-100 dark:border-slate-600">
                      ...
                      <Badge variant="secondary" className="bg-amber-200 dark:bg-slate-600 text-amber-800 dark:text-amber-200">
                        #{beers.length - index}
                      </Badge>
                    </div>
                    {index < beers.length - 1 && (
                      <Separator className="my-2 bg-amber-200 dark:bg-slate-600" />
                    )}
                  </div>
                ))}
              </div>
        ...
        {/* Footer */}
        <div className="text-center mt-8 text-amber-600 dark:text-amber-400">
          <p className="text-sm">
